...
greater element for each element of the array in order of their appearance in the array. Next greater 
element of an element in the array is the nearest element on the right which is greater than the 
current element. If there does not exist next greater of current element, then next greater 
...
int main()
{
    ...
    printf("Output for next greatest element : "); 
    for(i=0 ; i<n ; i++) 
    {
        int number = -1 ; 
        for(j=i ; j<n ; j++)
